...
- `layout.hold_workspaces` makes every saved numbered workspace persistent for the whole pass with `hl.workspace_rule({ workspace = 'N', persistent = true })` in one `eval`, and `layout.release_workspaces` switches the rules off in a `finally`. A rule added at runtime creates its workspace at once and has no handle but the one returned, kept in the Lua global `layout.HOLDS`; switched off, an empty workspace goes (checked on Hyprland 0.56.2, 2026-09-25). Without the hold, a setup that renumbers workspaces to close gaps moved workspace 3 into 2 while 2's windows were loading.
...
```sh
python3 -m unittest discover -s tests                                  # 388 tests, about a second
